...
export default function History() {
  ...
  const [showCatboxFiles, setShowCatboxFiles] = useState(false);
  const [catboxFiles, setCatboxFiles] = useState<any[]>([]);

  const userhash = localStorage.getItem("catbox_userhash") || "";
  const cookies = localStorage.getItem("catbox_cookies") || "";
  ...
  const fetchCatboxFiles = async () => {
    ...
    try {
      ...
      if (res.ok && data.files) {
        setCatboxFiles(data.files);
        setShowCatboxFiles(true);
      }
    } catch (error) {
      console.error("Failed to fetch catbox files:", error);
    }
  };
    ...
    <div className="history-page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Your recent uploads
          {userhash
            ? " — delete available with userhash"
            : " — set userhash in Settings to enable delete"}
          .
        </p>
      </div>
              ...
              return (
                ...
                >
                  <div className="history-card-thumb">
                    ...
                    <div
                      className={`history-card-icon ${thumb ? "hidden" : ""}`}
                    >
                      <svg
                        width="28"
                        height="28"
                  ...
                  <div className="history-card-actions">
                    <button
                      className={`hist-action-btn ${copiedUrl === item.url ? "copied" : ""}`}
